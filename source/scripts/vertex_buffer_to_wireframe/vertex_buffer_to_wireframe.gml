function vertex_buffer_to_wireframe(vbuff) {
	vertex_delete_buffer(_tempbuffer);
	_tempbuffer = vertex_create_buffer();
    vertex_begin(_tempbuffer, _vertexformat);
    ...
    while (buffer_tell(data) < buffer_get_size(data)) {
        ...
        vertex_add_point(_tempbuffer, ax, ay, az, anx, any, anz, atx, aty, make_colour_rgb(ar, ag, ab), aa);
        vertex_add_point(_tempbuffer, bx, by, bz, bnx, bny, bnz, btx, cty, make_colour_rgb(br, bg, bb), ba);
        vertex_add_point(_tempbuffer, cx, cy, cz, cnx, cny, cnz, ctx, cty, make_colour_rgb(cr, cg, cb), ca);
    }
    
    vertex_end(_tempbuffer);
    buffer_delete(data);

	...
	if(global._state == "game"){
		function seq_shader_begin() {
			if(event_type == ev_draw && event_number == 0){
				if(ds_map_exists(global._sequenceColors, layer_get_name(layer))){
					if(is_array(global._sequenceColors[? layer_get_name(layer)])){
		...
		for(var i = 0; i < array_length(seqkeys); i++){
			if(ds_map_exists(global._sequenceColors, seqkeys[i])){
				if(is_array(global._sequenceColors[? seqkeys[i]])){

			...
			if(_freeze <= 0){
				...
				if(!_death){
					...
					if(_groundtimer > 0){
						...
						if((!keyhold("left") && !keyhold("right") && !keyhold("menu_left") && !keyhold("menu_right")) || (keyhold("left") && keyhold("right")) || (keyhold("menu_left") && keyhold("menu_right"))){
							_xspd = 0;
					
							_holdL = false;
							_holdR = false;
						} else if(keyhold("left") || keyhold("menu_left")){
							if(goleft){
								_xspd = -_movespd;
								_yspd = 0;
					...
					switch(_plstate){
						...
						case LODE_STATE_LADDER:
							...
							if((!keyhold("up") && !keyhold("down") && !keyhold("menu_up") && !keyhold("menu_down")) || (keyhold("up") && keyhold("down")) || (keyhold("menu_up") && keyhold("menu_down"))){
								_yspd = 0;
							} else if(keyhold("up") || keyhold("menu_up")){
								if(!goleft){
									_xspd = 0;
									_yspd = -_movespd;
									var inst = instance_place(x,y,obj_lode_ladder);
						...
						case LODE_STATE_ROPE:
							...
							if(keypress("down") || keypress("menu_down")){

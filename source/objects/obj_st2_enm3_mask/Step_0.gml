{
	...
	if(!global._pause && !_inactive){
		...
		if(!_init){
			...
			if(_enmtype == 0){
				_gl_clapintervals = [15,36];
				
				if(_ailevel >= 5){
					_gl_clapintervals = [12,27];
				}
				if(_ailevel >= 7){
					_gl_clapintervals = [9,20];
				}
				if(_ailevel >= 9){
					_gl_clapintervals = [7,14];
				}
			}
			...
			if(!_idiot && _startTimer <= 0){
				if(!_sequence_finished){
					...
					switch(_gl_atkstate){
						case GL_ATK_NONE:
							if(_alt_tutorial && _enmtype == 1 && !_createprompt){
							...
							if(_freeze <= 0){
								...
								if(_enmtype == -1 && !_gl_spin && _gl_spinact == 0){
									add_trait(TRAIT_JABS);
								}
								...
								if(_enmtype == 1){
									...
									if(_gl_tongspin_act > 0){
										if(_taunt > 0 || _successparry > 0 || _shockwave || _falling || _hurttimer > 0 || _blocktimer > 0 || _dodgetimer > 0){
											if(_gl_tongspin_act > 0){
												_block_endzones = _block_endzones_start;
												_dodgezones = _dodgezones_start;
											}
									...
									if(!_mashed && _stuntimer <= 0 && !_death){
										_gl_tongspin_addtime = true;
									...
									switch(_gl_tongspin_act){
										case 1:
											...
											remove_trait([TRAIT_DODGE,TRAIT_GRAB,TRAIT_BLOCK]);
											//prep animation
											...
											if(_shockwave){
												...
												add_trait([TRAIT_DODGE,TRAIT_GRAB,TRAIT_BLOCK]);
											}
										break;
										case 2:
											_alt_attack = true;
											...
											if(place_meeting(x,y,obj_punchhitbox)){
												var hbox = instance_place(x,y,obj_punchhitbox);
												if(instance_exists(hbox)){
													if(hbox._ptype == "pl" && hbox._type == "air"){
														var val = 36;
														if(!_alt_tutorial){
															val = 32;
														}
														if(_gl_tongspin_amp < val){
															_gl_tongspin_amp = val;
														}
													}
												}
											}
											...
											if(_alt_tutorial){
												_gl_tongspin_amp += 0.3;
											} else {
												_gl_tongspin_amp += 0.7;
											}
											...
											if(!_gl_spindir){
												...
												if(round(_gl_tongspin_amp % 6) < 1){
													var im = instance_create_depth(x, y, depth+32, obj_enm_afterIM);
													...
													im._decaytime = 0.02;
													im._xspd = random_range(4,6)*_freedir;
													im._yspd = random_range(4,6)*_freedir_v;
												}
											}
											
											if(_gl_tongspin_amp >= 40){
												_atkallowed = [ATK_NORM,ATK_KO];
												remove_trait([TRAIT_DODGE,TRAIT_GRAB,TRAIT_HURT,TRAIT_GRAB]);
											
												_spin = true;
												_spinhits = 0;
												_falldecay = 1;
											
												_spinatk = instance_create_depth(x, y, -1, obj_punchhitbox);
												_spinatk._parentobj = self.id;
												_spinatk._ptype = "all";
												_spinatk._scale = [5.5, 4];
												_spinatk._offset = [0,-48];
												_spinatk._timer = 999;
												_spinatk._height = 0;
												_spinatk._type = "spin_enm";
												_spinatk._damage = ATK_KO;
												_spinatk._persist = true;
												_spinatk._canparry = false;
												_spinatk._delay = 45;
												...
												if(!_shockwave){
													if(_prompts != noone && instance_exists(_prompts)){
														...
														_prompts._prompt_type = "smackdown_t";
													}
												}
											}
											...
											if(_hurttimer > 0 || _falling){
												...
												if(_standup){
													...
													_block_endzones = ["upper","idle","crouch"];
													_dodgezones = [];
												}
											}
											...
											if(_shockwave){
												_gl_tongspin_act = 0;
												_atkallowed = [ATK_NORM,ATK_KO];
												add_trait([TRAIT_DODGE,TRAIT_BLOCK,TRAIT_MASHED]);
											}
										break;
										case 3:
											//spinning around
											...
											if(_enmtype == 1){
												...
												_fallxspd = ((spd+(_wallbonks*0.5))*_freedir)*_falldecay;
												_fallyspd = ((spd+(_wallbonks*0.5))*_freedir_v)*_falldecay;
											}
									
											//particles
											_gl_particle ++;
											if(_gl_particle >= 3){
												var p = instance_create_depth(x-52+random_range(-64,64),y+76+random_range(-64,64), depth, obj_particle);
												p._type = "gost_smoke"+string(choose(1,2,3));
												p._move = true;
												p._xspd = random_range(-5,5);
												p._yspd = random_range(-5,5);
												_gl_particle = 0;
											}
									
											//ending
											if(_height <= _groundlevel+12 && (_spinhits >= 2 || _shockwave > 0 || _wallbonks >= maxbonk || abs(_fallxspd) < 4 || _successparry > 0)){
												_gl_spinendtimer ++;
										
												_falldecay = lerp(_falldecay, 0, 0.02);
										
												if(_gl_spinendtimer >= 36){
													_block_endzones = _block_endzones_start;
													_dodgezones = _dodgezones_start;
										
													_spin = false;
										
													add_trait([TRAIT_HURT,TRAIT_STUN,TRAIT_GRAB,TRAIT_MASHED,TRAIT_DODGE,TRAIT_BLOCK]);
													_atkallowed = [ATK_NORM,ATK_KO];
											
													sfx_play_proximity(snd_gostdizzy);
												...
												_dodgezones = ["upper","crouch","slide"];
											}
										break;
									}
						...
						case GL_ATK_ALT1:
							...
							if(place_meeting(x,y,obj_punchhitbox)){
								var hbox = instance_place(x,y,obj_punchhitbox);
								if(instance_exists(hbox)){
									if(hbox._ptype == "pl" && hbox._damage == ATK_MASH){
										with(_displayobj){
											visible = true;
							...
							switch(_gl_atkact){
								case 0:
									...
									if(!_gl_atksmoke){
										sfx_play_proximity(snd_grinzy_warp1, 0.6);
										
										var smoke = instance_create_depth(x-32,y-32,_displayobj.depth-32, obj_st2_enm3_smoke);
										smoke._maxcolors = _maxcolors;
	
										smoke._mult_colorinArray = _mult_colorinArray;
										smoke._mult_coloroutArray =_mult_coloroutArray;
										smoke._mult_tolrArray = _mult_tolrArray;
										smoke._mult_blendArray = _mult_blendArray;
								...
								case 1:
									...
									if(_gl_atktimer >= _gl_clapintervals[0]){
										_dh = instance_nearest(x,y,obj_dh_mask);
										if(_dh != noone && instance_exists(_dh)){
											if(_gl_curclaps > 0){
												if(x < _dh.x){
													x = global._cameraX+WIDTH-24;
													_curdir = DIR_L;
												} else {
													x = global._cameraX+120;
													_curdir = DIR_R;
												}
								...
								case 2:
									...
									if(!_gl_atksmoke){
										...
										sfx_stop(snd_grinzy_warp2);
										sfx_play_proximity(snd_grinzy_warp2, 0.6);
										
										var smoke = instance_create_depth(x-32,y-32,_displayobj.depth-32, obj_st2_enm3_smoke);
										smoke._maxcolors = _maxcolors;
										smoke.sprite_index = spr_st2_enm3_grinzy_appear;
									...
									if(_gl_curclaps > 0){
										if(_gl_atktimer < max(2,10-_gl_clapintervals[1])){
											_gl_atkclap = false;
											_displayobj.image_index = 0;
										} else {
											if(!_gl_swishsnd){
										...
										if(_displayobj.image_index >= 3 && !_gl_atkclap){
											sfx_play_proximity(snd_grinzy_clap);
											sfx_pitch(snd_grinzy_clap,random_range(0.9,1.15));
											...
											var ghost = instance_create_depth(x+(96*_curdir)+offs, y+48, _displayobj.depth+12, obj_st2_enm3_ghost);
											ghost._curdir = _curdir;
											ghost._parentobj = self;
											
											ghost._maxcolors = _maxcolors;
											ghost._mult_colorinArray = _mult_colorinArray;
											ghost._mult_coloroutArray = _mult_coloroutArray;
											ghost._mult_tolrArray = _mult_tolrArray;
											ghost._mult_blendArray = _mult_blendArray;
											
											var p = instance_create_depth(x+(164*_curdir)+offs,y-42,_displayobj.depth-32,obj_particle);
											p._type = "fx6";
									...
									if(_gl_curclaps > 0){
										if(_gl_atktimer >= _gl_clapintervals[1]){
											_gl_atksmoke = false;
											_gl_atktimer = 0;
											_gl_atkact = 0;
										}
							...
							if(mashed || _death || _gl_atkend || _shockwave){
								with(_displayobj){
									visible = true;
									global._gameshadows[? _occupy_id][? "draw"] = true;
								}
					...
					if(_freeze <= 0){
						...
						if(_anim_prev != _anim){
							...
							if(compare_anim("tongspin2", "dizzy")){
								_displayobj.image_index = 0;
								_anim_tr_anim = "spin_dizzy";
								_anim_tr_init = false;
								_anim_transition = true;
							}
					...
					if(_allsounds != -1){
						if(spinstartsnd){
							if(!sfx_isplaying(snd_spinstart)){
								...
								sfx_pitch(snd_spinstart, random_range(3,3.5));
							}
						} else {
							if(sfx_isplaying(snd_spinstart)){

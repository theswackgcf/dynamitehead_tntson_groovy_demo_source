{	
	if(_init){
		...
		if(_thunder_alp > 0){
			draw_set_alpha(clamp(_thunder_alp,0,1));
			draw_rectangle(0,0,WIDTH,HEIGHT,false);
			draw_set_alpha(1);
		}
	
		for(var i = 0; i < sprite_get_info(_bgspr).num_subimages; i++){
			draw_sprite(_bgspr, i, _bgpos[i], 0);
			draw_sprite(_bgspr, i, _bgpos[i]+WIDTH, 0);
		}
		...
		if(_colorsinit){
			...
			draw_sprite_ext(spr_dh_intro1, floor(_dhframe) % sprite_get_info(spr_dh_intro1).num_subimages, _dhpos, (HEIGHT-260)+sin(_dhtime / 12)*(sin(_dhtime/12)*28), 1.25, 1.25, 0, #FFFFFF, 1);

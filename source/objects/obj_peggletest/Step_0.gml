...
if(_blowingup){
	//PEGGLE TERRORISM!
	if(_curblowuptimer <= 0){
		for(var i = array_length(_pegs) - 1; i >= 0; i--){
		    if(_pegs[i][2] == true){
				var p = instance_create_depth(_pegs[i][0] - 42, _pegs[i][1] + 6, depth-1, obj_particle);
				p._type = "explosion";
				p._scale = 0.1;
				sfx_play(snd_explosion, 1, false);
				with(obj_camera){
					_ampX = 8 + (other._blows / 1.7);
					_ampY = 8 + (other._blows / 1.7);
				}
				
				array_delete(_pegs, i, 1);
				_pegshit--;
				_blows++;
				break;
			}
		}
		_curblowuptimer = clamp(_blowuptimer - (_blows / 8), 3, infinity);
	}
	_curblowuptimer--;
